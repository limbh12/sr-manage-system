...
### 보안 및 데이터 보호
- **비밀번호 암호화**: BCrypt 해시 알고리즘을 사용하여 사용자 비밀번호를 안전하게 단방향 암호화하여 저장
- **개인정보 암호화**: AES-256 알고리즘을 사용하여 민감 정보(이름, 전화번호, 이메일)를 양방향 암호화하여 저장
- **대상 데이터**: 사용자 정보, SR 신청자 정보, OPEN API 현황조사 담당자 정보
- **자동 처리**: JPA Converter를 통한 투명한 암호화/복호화 지원
- *상세 내용은 [docs/JPA_CONVERTER.md](docs/JPA_CONVERTER.md)를 참조하세요.*
