...
## 2025-12-08 추가: 파일 업로드/다운로드 개선

### 개요
- OPEN API 현황조사(설문) 항목의 수신파일 업로드/다운로드 기능에서 발견된 문제를 수정했습니다. 주요 개선은 파일 저장 경로 고정화, 저장 파일명 규칙화, 다운로드 시 파일명 인코딩/제어문자 제거 처리입니다.

### 주요 변경 사항
- 백엔드: 업로드 저장 경로를 프로젝트 내 `data/uploads`로 고정하고, 저장 파일명을 `survey_{id}__{원본_정리명}` 형태로 저장하도록 변경했습니다. 또한 DB에는 정리된 원본 파일명(`receivedFileName`)을 저장합니다.
  - 파일: `backend/src/main/java/com/srmanagement/service/OpenApiSurveyService.java` (업로드/다운로드 로직 수정)
- 백엔드: 컨트롤러에 단일 파일 업로드/다운로드 엔드포인트 추가
  - 파일: `backend/src/main/java/com/srmanagement/controller/OpenApiSurveyController.java` (`POST /api/surveys/{id}/file`, `GET /api/surveys/{id}/download`)
- 프론트엔드: 다운로드 시 `Content-Disposition` 헤더 파싱을 견고하게 개선하여 제어문자 제거 및 `filename*` 처리(퍼센트 디코드) 추가
  - 파일: `frontend/src/services/surveyService.ts`
- 프론트엔드: 설문 폼 제출 로직에서 파일이 선택된 경우 생성/수정 후에 별도 업로드 API를 호출하도록 수정
  - 파일: `frontend/src/components/survey/SurveyForm.tsx`

### 검증 및 참고
- 업로드 파일은 프로젝트의 `backend/data/uploads` 폴더에 `survey_{id}__*` 형태로 저장됩니다.
- 다운로드 시 서버는 해당 접두사의 파일을 찾아 `Content-Disposition` 헤더에 DB에 저장된 파일명(`receivedFileName`)을 사용하여 attachment로 반환합니다.

### 관련 커밋
- `파일업로드-다운로드-개선` : 백엔드/프론트엔드 업로드/다운로드 로직 개선 및 폼 연동
